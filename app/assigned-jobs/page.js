...
const page = () => {
  ...
  const getEmpJobs = async () => {
    try {
      ...
      handleTotalPages(
        Math.ceil(data.getJobAssignedEmployee.employees.length / PAGE_COUNT)
      );
    } catch (error) {
                        ...
                        <tr key={item._id}>
                          ...
                          <td>
                            {item?.job.map((item, index) => (
                              <Link
                                key={index}
                                href={`/all-jobs/${item?.assigned_job?._id}`}
                              >
                                <span>
                                  {index + 1}. {item?.assigned_job?.title}
                                  <br />
                                </span>
                              </Link>
                            ))}
                          ...
                          <td>
                            {item?.job.map((item, index) => (
                              <>
                                <DateFormatter dateString={item.assignedOn} />
                                <br />
                              </>
                            ))}
                          </td>
                                    ...
                                    <div className="form-group">
                                      <div className="pl-0 mb-3">
                                        ...
                                        >
                                          ...
                                          {item?.job.map((cat) => (
                                            <option
                                              key={cat.assigned_job._id}
                                              value={cat.assigned_job._id}
                                            >
                                              {cat.assigned_job.title.slice(
                                                0,
                                                15
                                              )}
                                              ...
